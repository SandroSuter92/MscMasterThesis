...
path_wq = path + '/waterQuality1.csv'

#Load data
df_wq = pd.read_csv(path_wq)

#Replace '#NUM!' string with NA-Value
ammonia = df_wq['ammonia']
_ammonia = []
for i in ammonia:
    if i == '#NUM!':
        _ammonia.append(None)
    else:
        _ammonia.append(i)
df_wq['ammonia'] = _ammonia

is_safe = df_wq['is_safe']
_is_safe = []
for i in is_safe:
    if i == '#NUM!':
        _is_safe.append(None)
    else:
        _is_safe.append(i)
df_wq['is_safe'] = _is_safe

df_wq['ammonia'] = df_wq['ammonia'].astype(float)
df_wq['is_safe'] = df_wq['is_safe'].astype('int')

#Drop NA-Values
df_wq = df_wq.dropna()

#Draw 4 random samples of 500 rows
samples = []
sample_seed = np.random.randint(0, 100)
np.random.seed(sample_seed)
for i in range(500):
    samples.append(np.random.randint(0, 7900))
df_sample_1 = df_wq.iloc[samples]
...
for i in range(500):
    samples.append(np.random.randint(0, 7900))
df_sample_2 = df_wq.iloc[samples]
...
for i in range(500):
    samples.append(np.random.randint(0, 7900))
df_sample_3 = df_wq.iloc[samples]
...
for i in range(500):
    samples.append(np.random.randint(0, 7900))
df_sample_4 = df_wq.iloc[samples]
...
for i in range(4):
    ...
    X_train, X_test, y_train, y_test = train_test_split(df_sample[['aluminium', 'ammonia', 'arsenic',
    ...
    #Normalize data
    col_X_train = ['aluminium', 'ammonia', 'arsenic',
                   'barium', 'cadmium', 'chloramine',
                   'chromium', 'copper', 'flouride',
                   'bacteria', 'viruses', 'lead',
                   'nitrates', 'nitrites', 'mercury',
    ...
    path = "C:/Users/localadmin/Desktop/Thesis/Code/models/GPB/WaterQuality/outputs.csv"
...
for i in range(4):
    ...
    # Check wether path is existing or not & creating needed folders, if not existing
    path = "models/randomForrest/WaterQuality"
    ...
    # Param_grid for hyperparameter tuning (These grid results in 45 combinations
    param_grid = {'n_estimators': [10,100,1000,10000]}  # 100,1000,10000
    ...
    #Set path to store output-file
    path = "C:/Users/localadmin/Desktop/Thesis/Code/models/randomForrest/WaterQuality/outputs.csv"
...
for i in range(4):
    ...
    #Set path to store output-file
    path = "C:/Users/localadmin/Desktop/Thesis/Code/models/GP/WaterQuality/outputs.csv"
...
for i in range(4):
    ...
    # Check wether path is existing or not & creating needed folders, if not existing
    path = "models/Boosting/WaterQuality"
    ...
    #Set path to store output-file
    path = "C:/Users/localadmin/Desktop/Thesis/Code/models/Boosting/WaterQuality/outputs.csv"
